...
function downloadFileByForm(url, fieldData) {
	...
	form.append($("<input></input>").attr("type", "hidden").attr("name", "bj").attr("value", fieldData.studentType));
...
layui.use(['form','layer'], function () {
	...
	$('#exportConfirmedStudent').on('click', function () {
		...
		var url=contextPath +"/xm/exportBjRank";　
		$.fileDownload(url,{
		 data:{school:areaId,bj:studentType,xmid:xmid},
