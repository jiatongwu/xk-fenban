...
var batchesTypeListData = null;

function batchesTypeForVariable() {
	$.getJSON("/batch/allBatches",
		function (data) {
			batchesTypeListData = data;
		});
}
batchesTypeForVariable();

function batchesTypeReturnTypeName(typeId) {
	//console.log(batchesTypeListData);
	//console.log(typeId);
	var result = "";
	if (typeId !== undefined && typeId!=null) {
		
		$.each(batchesTypeListData, function (i, item) {
			var tmpId = item.batchCode;
			var tmpTypeId = typeId.toString();
			if (tmpId == tmpTypeId) {
				result = item.name;
				return;
...
layui.use(['jquery', 'form', 'table', 'layer', 'element'], function () {
	...
	table.render({
		elem: '#table',
		height: 'full-300',
		url: '/student/zhaoShengSchoolStudentTable',
		where: {
			name: ''
		},
		page: {
		...
		},
		{
			...
			title: '录取批次',
			templet: function (d) {
				return batchesTypeReturnTypeName(d.luquPiciId);
			}
		}, {
			field: 'type',
			title: '考生类型',
	...
	table.on('tool(table)', function (obj) {
		...
		if (event == 'edit') {
			...
			layer.open({
				...
				area: ['500px', '600px'],
				title: '查看考生录取信息',
				content: '/student/zhaoShengSchoolViewStudentPage?id=' + data.id,
				end: function () {
				}
			});
